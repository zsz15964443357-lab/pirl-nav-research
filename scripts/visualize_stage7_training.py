"""Generate Stage 7 training curve visualizations."""
...
from pirl_nav.visualization import generate_training_visuals, write_visual_index  # noqa: E402
...
def main() -> int:
    ...
    figures = generate_training_visuals(config)
    write_visual_index(config.visual_index_path, figures)
    print(f"wrote {len(figures)} Stage 7 training figures under {config.figures_dir}")
